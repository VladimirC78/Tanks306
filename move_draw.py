...
def bullet_move(obj,field):
    ...
    obj.r[1] += obj.v[1]
    for wall in walls:
        if wall.wall_hit(obj)["l"]==True:
            obj.v[0] *= -1
        elif wall.wall_hit(obj)["r"]==True:
            obj.v[0] *= -1
        if wall.wall_hit(obj)["u"]==True:
            obj.v[1] *= -1
        elif wall.wall_hit(obj)["d"]==True:
            obj.v[1] *= -1
...
def motion_up(obj,walls):        # движение вверх с учетом столкновения танка со стенами в карте(у танка теперь есть свой словарь)
    if obj.tank_check_hit(walls)["l"] == True:
        if np.sin(obj.ang) <= 0:
            obj.r[0] += obj.v * np.sin(obj.ang)
    if obj.tank_check_hit(walls)["r"] == True:
        if np.sin(obj.ang) >= 0:
            obj.r[0] += obj.v * np.sin(obj.ang)
    if (obj.tank_check_hit(walls)["r"] == obj.tank_check_hit(walls)["l"]) and obj.tank_check_hit(walls)["r"] == False:
        obj.r[0] += obj.v * np.sin(obj.ang)

    if obj.tank_check_hit(walls)["u"] == True:
        if np.cos(obj.ang) <= 0:
            obj.r[1] += obj.v * np.cos(obj.ang)
    if obj.tank_check_hit(walls)["d"] == True:
        if np.cos(obj.ang) >= 0:
            obj.r[1] += obj.v * np.cos(obj.ang)
...
def motion_down(obj,walls):
    ...
    if obj.tank_check_hit(walls)["r"] == True:
        if np.sin(obj.ang) <= 0:
            obj.r[0] -= obj.v * np.sin(obj.ang)
    if (obj.tank_check_hit(walls)["r"] == obj.tank_check_hit(walls)["l"]) and obj.tank_check_hit(walls)[
        "r"] == False:
        obj.r[0] -= obj.v * np.sin(obj.ang)
    ...
    if obj.tank_check_hit(walls)["d"] == True:
        if np.cos(obj.ang) <= 0:
            obj.r[1] -= obj.v * np.cos(obj.ang)
    if (obj.tank_check_hit(walls)["u"] == obj.tank_check_hit(walls)["d"]) and obj.tank_check_hit(walls)[
        "d"] == False:
        obj.r[1] -= obj.v * np.cos(obj.ang)
